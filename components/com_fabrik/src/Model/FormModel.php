...
class FormModel extends FabrikSiteModel
{
	 ...
	 *
	 * @return  GroupModel[]  Group model objects with table row loaded
	 *
	 * @since 4.0
	 */
	...
	public function setInsertId($insertId)
	{
		...
		$this->formData[FStringHelper::shortColName($item->db_primary_key)] = $insertId;
		$this->formData[FStringHelper::shortColName($item->db_primary_key) . '_raw'] = $insertId;
	 ...
	 *
	 * @return  ListModel  fabrik list model
	 *
	 * @since 4.0
	 */
	...
	public function getTable($name = '', $prefix = 'Table', $options = array())
	{
		...
		{
			$this->form = parent::getTable(FormTable::class);
		}
	 ...
	 * @param   string  $key                 What value should be used for the option value 'name' (default) or 'id' * @since 4.0.7
	 ...
	 * @param   array  $opts    Property 'includePublished' can be set to 0; * @since 4.0.7
	 *                          Property 'loadPrefilters' * @since 4.0.7.1 - used to ensure that pre-filter elements are loaded in inline edit
	 *
	...
	public function getElementIds($ignore = array(), $opts = array())
	{
		...
		if (FArrayHelper::getValue($opts, 'loadPrefilters', false))
		{
			...
			foreach ($afilterFields as $name)
			{
				...
				$name = $name ? FStringHelper::rtrimword($name, '_raw') : $name;
		...
		foreach ($groups as $gid => $groupModel)
		{
			...
			foreach ($elementModels as $elementModel)
			{
				...
				if ($key != 'id')
				{
					...
					if ($this->addDbQuote)
					{
						$val = FStringHelper::safeColName($val);
					}
	...
	public function paginateRowId($dir)
	{
		...
		$query = $db->getQuery(true);
		$query->select($item->db_primary_key . ' AS ' . FStringHelper::safeColNameToArrayKey($item->db_primary_key))->from($item->db_table_name)
	...
	public function isNewRecord()
	{
		...
		else
		{
			...
			if ($this->app->input->get('task', '') === 'form.process')
			{
				...
				$useKey = Worker::getMenuOrRequestVar('usekey', '', $this->isMambot, 'var', $opts);
	...
	protected function getMaxRowId()
	{
		...
		$fabrikDb = $listModel->getDb();
		$item = $listModel->getTable();
		$k = FStringHelper::safeNameQuote($item->db_primary_key);

		// @TODO JQuery this
		$fabrikDb->setQuery("SELECT MAX($k) FROM " . FStringHelper::safeColName($item->db_table_name) . $listModel->buildQueryWhere());
	...
	public function getData()
	{
		...
		{
			$test_key = FStringHelper::rtrimword($key, '_raw');
			...
			else
			{
				...
				if (!$sessionLoaded)
				{
					...
					if (!empty($useKey) || $this->rowId !== '')
					{
						...
						if (is_null($rows))
						{
							throw new \RuntimeException($fabrikDb->getErrorMsg());
						}
						...
						if (empty($rows) && $this->rowId != '')
						{
							...
							else
							{
								...
								if (empty($useKey) && !$this->isMambot && in_array($input->get('view'), array('form', 'details')))
								{
									$this->rowId = '';
									/**
									 * @todo - figure out what to do with this now that JError is no more
									 *
	...
	public function buildQuery($opts = array())
	{
		...
		if ($useKey != '')
		{
			...
			foreach ($useKey as &$tmpK)
			{
				...
				$tmpK = FStringHelper::safeColName($tmpK);
			}
	...
	public function parseIntroOutroPlaceHolders($text)
	{
		...
		$text = FStringHelper::translate($text);
