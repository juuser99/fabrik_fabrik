 ...
 * @since   4.0
 */
class ListFilterModel extends FabrikSiteModel
{
	/**
	 * Request
	 *
	 ...
	 * @since 4.0
	 */
	public function setListModel(ListModel $model)
	{
	...
	private function getSearchFormFilters(&$filters)
	{
		...
		if (!empty($fromFormId))
		{
			...
			{
				$fromForm = FabrikModel::getInstance(FormModel::class);
	...
	private function getQuerystringFilters(&$filters)
	{
		...
		{
			$oldKey = $key;
			$key = FStringHelper::safeColName($key);
			...
			if (substr($oldKey, -4, 4) == '_raw')
			{
				...
				$key = FStringHelper::safeColName(FStringHelper::rtrimword($oldKey, '_raw'));
			}

			$elementModel = $formModel->getElement(FStringHelper::rtrimword($oldKey, '_raw'), false, false);
	...
	private function indQueryString($elementModel, &$filters, $value, $condition, $join, $grouped, $eval, $key, $raw = false)
	{
		...
		if (is_string($value))
		{
			...
			$value = Worker::replaceWithUserData($value);
		}

		$k2 = FStringHelper::safeColNameToArrayKey($key);
		/**
