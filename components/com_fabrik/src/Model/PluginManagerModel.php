...
use Joomla\Component\Fabrik\Site\Plugin\FabrikPlugin;
...
class PluginManagerModel extends FabrikSiteModel
{
	 ...
	 * @since    4.0
	 */
	protected $abstractPlugins = array();
	...
	protected function &loadPlugInGroup($group)
	{
		...
		foreach ($plugins as $plugin)
		{
			$this->abstractPlugins[$group][$plugin->name] = $plugin;
		}

		return $this->abstractPlugins[$group];
	}
	...
	public function runPlugins($method, $parentModel, $type = 'form')
	{
		...
		if ($type == 'form')
		{
			 ...
			 */
			$tmethod   = 'onForm' . FStringHelper::ltrimword($method, 'on');
