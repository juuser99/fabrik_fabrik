...
class AbstractElementDatabaseJoinPlugin extends AbstractElementListPlugin
{
	...
	public function getAsField_html(&$aFields, &$aAsFields, $opts = array())
	{
		...
		if ($params->get('join_conn_id') == $connection->get('id') || $element->plugin != 'databasejoin')
		{
			...
			*$k = FStringHelper::safeColName("`$join->keytable`.`$element->name`");
			*/
			$keyTable = isset($join->keytable) ? $join->keytable : $join->join_from_table;
			$k        = FStringHelper::safeColName($keyTable . '.' . $element->name);
	...
	public function getRawColumn($useStep = true)
	{
		...
		return $useStep ? $k . '___' . $name : FStringHelper::safeColName($k . '.' . $name);
	}
	 ...
	 * @param   JoinTable $join   Join table
	 ...
	 * @since 4.0
	 */
	protected function parseThisTable($string, JoinTable $join = null, $alias = null)
	{
		if (is_null($join))
		{
			$join = $this->getJoin();
		...
		return str_replace('{thistable}', FStringHelper::safeNameQuote($alias), $string);
	}
	...
	protected function getJoinLabel()
	{
		...
		$label = FStringHelper::shortColName($join->params->get('join-label'));

		if ($label == '')
		{
			if (!$this->isJoin())
			{
				Log::add('db join: Could not find the join label for ' .
					$this->getElement()->get('name') . ' try unlinking and saving it', Log::WARNING, 'jerror');
			}
	...
	protected function emptyConcatString($label)
	{
		...
		for ($i = 0; $i < $countBits; $i++)
		{
			...
			{
				$bits[$i] = FStringHelper::ltrimword($bits[$i], "'");
				$bits[$i] = FStringHelper::rtrimword($bits[$i], "'");
			}
		}
	 ...
	 *
	 * @param   DatabaseQuery $query BuildQuery
	 ...
	 * @since 4.0
	 */
	protected function buildQueryDescription(DatabaseQuery $query, $data)
	{
		$params = $this->getParams();
		...
		if ($desc !== '')
		{
			...
			$desc = FStringHelper::isConcat($desc) ? $desc : $db->qn($desc);
			$desc = "REPLACE(" . $desc . ", '\n', '<br />')";
	 ...
	 *
	 * @param   DatabaseQuery $query false to return string, or JQueryBuilder object
	 *
	 * @since 3.0rc1
	 *
	 ...
	 * @param   array              $opts           Options
	 * @param   bool|DatabaseQuery $query          Append where to JDatabaseQuery object or return string (false)
	 *
	...
	protected function buildQueryWhere($data = array(), $incWhere = true, $thisTableAlias = null, $opts = array(), $query = false)
	{
		...
		{
			$where = FStringHelper::ltrimword($where, 'WHERE', true);
			$query->where($where);
	 ...
	 *
	 * @return    ConnectionTable    connection table
	 *
	 * @since 4.0
	 */
	protected function loadConnection()
	{
		...
		// Alan - this can't work as there is not a getConnection method defined on ListModel
		/**
	...
	protected function renderDescription($options = array(), $default = array())
	{
		...
		if ($params->get('join_desc_column', '') !== '')
		{
			...
			Html::jLayoutJs($descDivLayout, $descDivLayout, $displayData, array($this->layoutBasePath()));
	...
	protected function renderCheckBoxList($data, $repeatCounter, &$html, $tmp, $default)
	{
		...
		$name = FStringHelper::rtrimword($name, '[]');
	...
	{
		$params   = $this->getParams();
		$elName   = FStringHelper::safeColNameToArrayKey($this->getFilterFullName());
		$return   = array();
	...
	{
		$joinTable     = FStringHelper::safeColName($this->getDbName());
		$join          = $this->getJoin();
		$joinTableName = FStringHelper::safeColName($join->table_join_alias);
		$joinKey       = $this->getJoinValueColumn();
		$elName        = FStringHelper::safeColName($this->getFullName(true, false));
	...
	protected function filterValueList_All($normal, $tableName = '', $label = '', $id = '', $incjoin = true)
	{
		...
		*/
		...
		$elementName    = FStringHelper::safeColName($this->getFullName(false, false));
	...
	protected function getJoinValueFieldName()
	{
		...
		return FStringHelper::shortColName($params->get('join_key_column'));
	}
		...
		else
		{
			if ($type == 'querystring')
			{
				...
				*/
				if (!$this->_rawFilter)
				{
					$k = $this->db->qn($params->get('join_db_name')) . '.' . $this->db->qn($this->getLabelParamVal());
				}
				else
				{
					$k = $key;
		...
		if (!$this->_rawFilter && ($type === 'searchall' || ($type === 'prefilter') || $type === 'menuPrefilter'))
		{
			if ($type !== 'prefilter' && $type !== 'menuPrefilter')
			{
				if (!$this->isJoin())
				{
					...
					{
						$key = FStringHelper::safeColName($join->table_join_alias) . '.' . $this->db->qn($key);
					}
				}
			}
			else
			{
				if (!$this->_rawFilter)
				{
					$key = $this->db->qn($params->get('join_db_name')) . '.' . $this->db->qn($this->getLabelParamVal());
				}
			}
		...
		else
		{
			...
			else
			{
				...
				if ($this->isJoin())
				{
					if ($fType == 'field')
					{
						$where = $this->db->qn($dbName . '.' . $this->getLabelParamVal());
					}
					else
					{
						$where = $this->db->qn($dbName . '.' . $this->getJoinValueFieldName());
					}

					$groupBy = $this->db->qn($dbName . '.parent_id');
				...
				else
				{
					if ($fType === 'auto-complete')
					{
						...
						$key = $this->db->qn($this->getFullName(false, false));
					}
	...
	protected function checkboxRows($groupBy = null, $condition = null, $value = null, $where = null, $offset = 0, $limit = 0)
	{
		...
		$sql     = (string) $query;
		$groupBy = FStringHelper::shortColName($groupBy);
	...
	public function getFilterFullName()
	{
		...
		return FStringHelper::safeColName($elName);
	}
	...
	protected function getLabelParamVal()
	{
		...
		$label               = FStringHelper::shortColName($label);
	...
	public function onAjax_getOptions()
	{
		...
		if ($groupModel->isJoin())
		{
			if ($groupModel->canRepeat())
			{
				...
				foreach ($groupElements as $elementPlugin)
				{
					$name                    = $elementPlugin->getFullName(true, false);
	...
	protected function updateFabrikJoins($data, $tableJoin, $keyCol, $label)
	{
		...
		{
			$elementPlugin    = Worker::getPluginManager()->getElementPlugin($id);
			$data['group_id'] = $elementPlugin->getElement()->get('group_id');
			$data['id']       = $id;
	 ...
	 *
	 * @param   AbstractElementPlugin|PlgFabrik_ElementDatabasejoin $elementPlugin element model
	...
	public static function cacheAutoCompleteOptions(AbstractElementPlugin $elementPlugin, $search, $opts = array())
	{
		...
		$c      = $elementPlugin->getLabelOrConcatVal();

		if (!strstr($c, 'CONCAT'))
		{
			$c = FStringHelper::safeColName($c);
		}

		$filterMethod = $elementPlugin->getFilterBuildMethod();

		if ($filterMethod == 1)
		{
			$join      = $elementPlugin->getJoin();
			$joinTable = $join->table_join_alias;
			$opts      = array();
			...
			return parent::cacheAutoCompleteOptions($elementPlugin, $search, $opts);
		}
		// $$$ hugh - added 'autocomplete_how', currently just "starts_with" or "contains"
		// default to "contains" for backward compat.
		$elementPlugin->autocomplete_where = $elementPlugin->_autocompleteWhere($params->get('dbjoin_autocomplete_how', 'contains'), $c, $search);
		$opts                              = array('mode' => 'filter');
		$tmp                               = $elementPlugin->_getOptions(array(), 0, true, $opts);
	...
	public function getOrderByName()
	{
		...
		{
			$return = strstr($joinVal, '___') ? FStringHelper::safeColName($joinVal) : $joinTable . '.' . $joinVal;
		}
		else
		{
			$return = $joinVal;
	...
	public function buildQueryElementConcat($jKey, $addAs = true)
	{
		...
		$fullElName = $this->db->qn($this->getFullName(true, false));
