 ...
 * @since    4.0
 */
class AbstractElementListPlugin extends AbstractElementPlugin
{
	/**
	...
	protected function filterQueryMultiValues($key, $condition, $originalValue, $evalFilter, $type)
	{
		...
		switch ($condition)
		{
			...
			case 'NOT IN':
				...
				{
					$v = trim($v);
					$v = FStringHelper::ltrimword($v, '"');
					$v = FStringHelper::ltrimword($v, "'");
					$v = FStringHelper::rtrimword($v, '"');
					$v = FStringHelper::rtrimword($v, "'");
				}
				break;
			default:
		...
		{
			$v2    = str_replace("/", "\\\\/", $v2);
			$str[] = '(' . $key . $partialComparison . $this->db->q('%"' . $v2 . '"%') . $glue . $key .
				$comparison . $this->db->q($v2) . ') ';
		}
	...
	public static function cacheAutoCompleteOptions(AbstractElementPlugin $elementPlugin, $search, $opts = array())
	{
		...
		$rows  = $elementPlugin->filterValueList(true, '', $label);
