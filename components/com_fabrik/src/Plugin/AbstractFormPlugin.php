...
abstract class AbstractFormPlugin extends FabPlugin
{
	...
	public function getEmailData()
	{
		...
		foreach ($groups as $gKey => $groupModel)
		{
			...
			if ($groupModel->canRepeat())
			{
				if ($groupModel->isJoin())
				{
					...
					if (is_object($joinTable))
					{
						...
						reset($elementPlugins);

						if (!empty($elementPlugins))
						{
							$tmpElement        = current($elementPlugins);
			...
			for ($c = 0; $c < $repeatGroup; $c++)
			{
				...
				{
					// Force reload?
					$elementPlugin->defaults          = null;
					$elementPlugin->_repeatGroupTotal = $repeatGroup - 1;

					$k   = $elementPlugin->getFullName(true, false);
					$key = $elementPlugin->getFullName(true, false);

					// Used for working out if the element should behave as if it was
					// in a new form (joined grouped) even when editing a record
					$elementPlugin->inRepeatGroup = $groupModel->canRepeat();
					$elementPlugin->_inJoin       = $groupModel->isJoin();
					$elementPlugin->setEditable(false);
					...
					if ($groupModel->isJoin())
					{
						if ($groupModel->canRepeat())
						{
							...
							$this->emailData[$k][$c]          = $elementPlugin->getEmailValue($raw, $model->formDataWithTableName, $c);
							continue;
						}
						else
						{
							...
							if (array_key_exists($k, $model->formDataWithTableName) && is_array($model->formDataWithTableName[$k]))
							{
								foreach ($model->formDataWithTableName[$k] as $multiKey => $multiData)
								{
									...
									$this->emailData[$k][$multiKey]          = $elementPlugin->getEmailValue($multiData, $model->formDataWithTableName, $multiData);
								}
								continue;
							}
						}
					 ...
					 */
					//if (!$elementPlugin->isJoin())
					//{
					$this->emailData[$k] = $elementPlugin->getEmailValue($emailValue, $model->formDataWithTableName, $c);
					//}
				}
			}
		}
	...
	public function getFieldValue($pName, $data, $default = '')
	{
		...
		$elementPlugin = Worker::getPluginManager()->getElementPlugin($params->get($pName));
	...
	public function getFieldName($pname, $short = false)
	{
		...
		if (is_numeric($value))
		{
			$elementPlugin = Worker::getPluginManager()->getElementPlugin($value);

			return $short ? $elementPlugin->getElement()->name : $elementPlugin->getFullName();
		}
		else
		{
			return $short ? FStringHelper::shortColName($value) : FStringHelper::safeColNameToArrayKey($value);
		}
	}
