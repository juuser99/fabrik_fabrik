...
class BaseView extends AbstractView
{
	...
	public function display($tpl = null)
	{
		...
		$this->table->label         = FStringHelper::translate($w->parseMessageForPlaceHolder($item->label, $_REQUEST));
		$this->table->intro         = $params->get('show_into', 1) == 0 ? '' : FStringHelper::translate($w->parseMessageForPlaceHolder($item->introduction));
		$this->table->outro         = $params->get('show_outro', 1) == 0 ? '' : FStringHelper::translate($w->parseMessageForPlaceHolder($params->get('outro')));
	 ...
	 * @since 4.0
	 */
	protected function access(ListModel $model)
	{
		if (!$model->canPublish())
		{
	...
	protected function buttons()
	{
		...
		$this->buttons->groupby    = Html::image('list-view', 'list', $this->tmpl, $buttonProperties);

		unset($buttonProperties['title']);
		$buttonProperties['alt'] = Text::_('COM_FABRIK_FILTER');
		$this->buttons->filter   = Html::image('filter', 'list', $this->tmpl, $buttonProperties);
	...
	protected function loadTemplateBottom()
	{
		...
		$reffer             = FStringHelper::removeQSVar($reffer, 'fabrik_incsessionfilters');
