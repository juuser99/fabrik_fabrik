...
<?php endif;?>
	...
	<table class="table table-striped">
		...
		<?php foreach ($this->items as $i => $item) :
			...
			$link = JRoute::_('index.php?option=com_fabrik&view=form&layout=edit&id='. $item->view);
			$canCreate	= $user->authorise('core.create',		'com_fabrik.form.1');
			$canEdit	= $user->authorise('core.edit',			'com_fabrik.form.1');
			$canCheckin	= $user->authorise('core.manage',		'com_checkin') || $item->form->checked_out==$user->get('id') || $item->form->checked_out==0;
			...
			<tr class="row<?php echo $i % 2; ?>">
					...
					<td>
						...
						if ($item->checked_out && ( $item->checked_out != $user->get('id'))) {
							echo  $item->form->label;
						} else {
						?>
						<a href="<?php echo $link; ?>">
							<?php echo $item->form->label; ?>
						</a>
					<?php } ?>
					</td>
					<td>
