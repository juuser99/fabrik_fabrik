...
class ElementModel extends FabAdminModel
{
	 ...
	 * @since 4.0
	 */
	public function getTable($type = ElementTable::class, $prefix = '', $config = array())
	{
		$config['dbo'] = Worker::getDbo(true);
	...
	public function validate($form, $data, $group = null)
	{
		...
		// Standard Form validation failed so we shouldn't test further as we can't be sure of the data
		if (!$ok)
		{
			return false;
		}
	...
	public function save($data)
	{
		...
		$params                = $data['params'];
		$data['name']          = FStringHelper::iclean($data['name']);
		$name                  = $data['name'];
		...
		if ($row->name === FStringHelper::shortColName($item->db_primary_key))
		{
			if ($name !== $row->name)
			{
		...
		foreach ($validation_plugins as $plugin)
		{
			...
			Form::addFormPath(JPATH_SITE . '/plugins/fabrik_validationrule/' . $plugin->get('pluginName'));
			$xmlFile = JPATH_SITE . '/plugins/fabrik_validationrule/' . $plugin->get('pluginName') . '/forms/fields.xml';
			$xml     = $plugin->Form->loadFile($xmlFile, false);
	...
	protected function updateJavascript($data)
	{
		...
		$eEvent     = FArrayHelper::getValue($Form, 'js_e_event', array());
		$eTrigger   = FArrayHelper::getValue($Form, 'js_e_trigger', array());
		$eCond      = FArrayHelper::getValue($Form, 'js_e_condition', array());
		$eVal       = FArrayHelper::getValue($Form, 'js_e_value', array());
		$ePublished = FArrayHelper::getValue($Form, 'js_published', array());
		$action     = (array) FArrayHelper::getValue($Form, 'action', array());
	...
	public function createRepeatElement($elementModel, $row)
	{
		...
		$opts->pk       = FStringHelper::safeQuoteName($tableName . '.id');
