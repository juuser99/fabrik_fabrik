...
class GroupModel extends FabrikAdminModel
{
	 ...
	 *
	 * @return  GroupTable|Table    A database object
	 *
	 * @since 4.0
	 */
	public function getTable($type = GroupTable::class, $prefix = '', $config = array())
	{
	...
	protected function checkRepeatAndPK($data)
	{
		...
		{
			if (FStrigHelper::safeColName($elementModel->getFullName(false, false)) == $pk)
			{
				return false;
			}
		}
	...
	public function makeJoinedGroup(&$data)
	{
		...
		foreach ($elements as $element)
		{
			...
			{
				$str   = FStrigHelper::safeColName($fName);
		...
		$jData = array('list_id' => $list->id, 'element_id' => 0, 'join_from_table' => $list->db_table_name, 'table_join' => $newTableName,
			'table_key' => FStrigHelper::shortColName($list->db_primary_key), 'table_join_key' => 'parent_id', 'join_type' => 'left',
			'group_id' => $data['id']);
