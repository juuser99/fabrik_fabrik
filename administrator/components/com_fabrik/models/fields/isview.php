...
class JFormFieldListfields extends JFormFieldList
{
	...
	private function js($res = array())
	{
		...
		$opts->repeat = $repeat;
		$opts->showAll = (int) ArrayHelper::getValue($this->element, 'showall', '1');
		$opts->highlightpk = (int) $highlightpk;
		$opts->mode = $mode;
		$opts->defaultOpts = $res;
		$opts->addBrackets = (bool) ArrayHelper::getValue($this->element, 'addbrackets', false);
	...
	{
		$str = array();
		$modeField = (string) ArrayHelper::getValue($this->element, 'modefield', 'textarea');
