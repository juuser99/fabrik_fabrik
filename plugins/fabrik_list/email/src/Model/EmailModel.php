...
namespace Fabrik\Plugin\FabrikList\Email\Model;
 ...
 * @since       4.0
 */
class EmailModel extends AbstractListPlugin
{
	/**
	 * Button prefix
	 *
	...
	public function getToField()
	{
		...
		elseif ($toType == 'table' || $toType == 'table_picklist')
		{
			...
			/** @var ListModel $toTableModel */
			$toTableModel = FabrikModel::getInstance(ListModel::class);
			$toTableModel->setId($table);
			$toDb = $toTableModel->getDb();

			$tableName          = FStringHelper::safeColName($tableName);
			$tableEmail         = FStringHelper::safeColName($tableEmail);
	...
	public function getRecords($key = 'ids', $allData = false)
	{
		...
		$pk2 = FStringHelper::safeColNameToArrayKey($pk) . '_raw';
	...
	private function _upload()
	{
		...
		foreach ($files as $file)
		{
			...
			{
				throw new \RuntimeException(Text::_('PLG_LIST_EMAIL_ERR_CANT_UPLOAD_FILE'));
			}
			else
			{
				$this->filepath[] = $path;
	...
	private function _send($row, $mailTo)
	{
		...
		$coverMessage  = FStringHelper::safeNl2br($coverMessage);
	...
	private function _evalTo($row)
	{
		...
		if ($toType == 'eval')
		{
			...
			$to     = Html::isDebug() ? eval($php) : @eval($php);
			Worker::logEval($to, 'Eval exception : listEmail::_evalTo() : %s');
		}
	...
	private function _thisMsg($row)
	{
		...
		$w             = new Worker;
		$input         = $this->app->input;
		$coverMessage  = FStringHelper::safeNl2br($input->get('message', '', 'html'));
		$emailTemplate = $this->_emailTemplate();
		$oldStyle      = $this->_oldStyle();
