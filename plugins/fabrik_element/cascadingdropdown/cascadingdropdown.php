 ...
 * @since       3.0
 */
class PlgFabrik_ElementCascadingdropdown extends AbstractElementDatabaseJoinPlugin
{
	/**
	...
	protected function buildQuery($data = array(), $incWhere = true, $opts = array())
	{
			...
			if (is_array($whereVal))
			{
				foreach ($whereVal as &$v)
				{
					...
					if (is_array($v))
					{
						foreach ($v as &$vchild)
						{
							$vchild = FStringHelper::safeQuote($vchild);
						}
						$v = implode(',', $v);
					}
					else
					{
						$v = FStringHelper::safeQuote($v);
					}
				}
		...
		{
			$val = FStringHelper::safeColName($params->get($this->labelParam));
		...
		if ($desc !== '')
		{
			$query->select(FStringHelper::safeColName($desc) . ' AS description');
		}

		$query->from($db->qn($table) . ' AS ' . $db->qn($join->table_join_alias));
		$query = $this->buildQueryJoin($query);
		$where = FStringHelper::rtrimword($where);
	...
	protected function queryKey()
	{
		...
		$table  = $this->getDbName();
		$params = $this->getParams();
		$key    = FStringHelper::safeColName($params->get('cascadingdropdown_id'));
	...
	protected function getJoinValueFieldName()
	{
		...
		return FStringHelper::shortColName($full);
	}

	/**
	 * Get the observed element's element model
	 *
	 * @return AbstractElementPlugin|bool element model or false
	 *
	 * @since 4.0
	 */
	...
	public function finalCopyCheck($elementMap)
	{
		...
		if (!$element->store())
		{
			throw new \RuntimeException($element->getError());
		}
	}
