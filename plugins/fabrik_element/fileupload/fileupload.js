...
var FbFileUpload = my.Class(FbFileElement, {
    ...
    watchBrowseButton: function () {
        if (window.File && window.FileReader && window.FileList && window.Blob) {
            $('#' + this.element.id).on('change', function (evt) {
        ...
        if (b.length > 0) {
            b.on('click', function (e) {
                ...
                if (window.confirm(Joomla.JText._('PLG_ELEMENT_FILEUPLOAD_CONFIRM_SOFT_DELETE'))) {
                    ...
                    var deleteSpan = $('#' + this.element.id + '_delete_span');
                    if (deleteSpan) {
                        deleteSpan.destroy();
                    }
                }
    ...
    decloned: function (groupid) {
        ...
        var i = f.find('input[name=fabrik_deletedimages[' + groupid + ']');
        if (typeOf(i) === 'null') {
