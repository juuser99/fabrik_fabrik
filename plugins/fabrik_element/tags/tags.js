...
var FbTags = my.Class(FbElement, {
    ...
    setUp: function () {
        ...
        this.options.element.chosen({
            disable_search_threshold: 10,
            allow_single_deselect   : true
        });

        this.sel = this.options.element.ajaxChosen({
            type          : 'GET',
        ...
        this.sel.on('change', function () {
            ...
            $(sel.data().chosen.results_data).each(function () {
                $(opts[this.options_index]).attr('selected', this.selected);
            });
        });
