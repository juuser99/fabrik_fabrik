...
var FbGoogleMapViz = my.Class({
    ...
    constructor: function (element, options) {
        ...
        if (typeOf(this.container) !== 'null') {
            var form = this.container.find('form[name=filter]');
            ...
            // Watch filter submit
            var submit = this.container.find('input.fabrik_filter_submit');
        ...
        if (this.options.use_overlays) {
            this.options.overlay_urls.each(function (overlay_url, k) {
                ...
                this.options.overlay_events[k] = function (e) {
                    self.toggleOverlay(e);
                };
                $('#overlay_chbox_' + k).on('click', this.options.overlay_events[k]);
            }.bind(this));
        }
    },
    ...
    renderGroupedSideBar: function () {
        ...
        this.grouped = {};
        c = $('#' + this.options.container).find('.grouped_sidebar');
        if (c.length === 0) {
            return;
        }
        c.empty();
        ...
        this.options.icons.each(function (i) {
            if (typeOf(this.grouped[i.groupkey]) === 'null') {
                ...
                var d = $(document.createElement('div')).html(label);
                if (d.find('a')) {
                    d = d.find('a');
                }
                linkText = d.get('html');
                ...
                var a = $(document.createElement('a'))
                    ...
                    .addClass('groupedLink' + k)
                    .on('click', function (e) {
                        var cname = e.target.className.replace('groupedLink', 'groupedContent');
                        cname = cname.split(' ')[0];
                        document.find('.groupedContent').hide();
                        document.find('.' + cname).show();
                    })
                    .html(linkText);
        ...
        if (!this.watchingSideBar) {
            c.on('click', 'a', function (event, clicked) {

                // Don't follow the link
                event.preventDefault();
                self.toggleGrouped(clicked);
            });
