...
class GooglemapModel extends AbstractVisualizationModel
{
	...
	protected function getPolyline()
	{
		...
		{
			$k = FStringHelper::safeColName(FStringHelper::rtrimword($polyElements[$c], '[]'));
	...
	private function getCordsFromData($d)
	{
		...
		if (strstr($v, ","))
		{
			...
			$v[1] = FStringHelper::rtrimword($v[1], ")");
		}
		else
		{
			$v = array(0, 0);
	...
	public function getJSIcons()
	{
		...
		foreach ($listIds as $listId)
		{
			...
			/*
			if (FStringHelper::usesElementPlaceholders($template))
			{
			...
			foreach ($data as $groupKey => $group)
			{
				foreach ($group as $row)
				{
					...
					if ($template_nl2br)
					{
						...
						$html = str_replace(array("\n", "\r"), "<br />", $html);
						*/
						$html = FStringHelper::safeNl2br($html);
					}
