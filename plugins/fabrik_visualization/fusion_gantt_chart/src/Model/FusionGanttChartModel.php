...
namespace Fabrik\Plugin\FabrikVisualization\FusionGanttChart\Model;
 ...
 * @since       4.0
 */
class FusionGanttChartModel extends AbstractVisualizationModel
{
	/**
	 * @var \FusionCharts
	 * @since 4.0
	...
	public function getChart()
	{
		...
		$process      = FStringHelper::safeColNameToArrayKey($process);
		$processRaw   = $process . '_raw';
		$start        = $params->get('fusion_gantt_chart_startdate');
		$start        = FStringHelper::safeColNameToArrayKey($start);
		$startRaw     = $start . '_raw';
		$end          = $params->get('fusion_gantt_chart_enddate');
		$end          = FStringHelper::safeColNameToArrayKey($end);
		$endRaw       = $end . '_raw';
		$label        = $params->get('fusion_gantt_chart_label');
		$label        = FStringHelper::safeColNameToArrayKey($label);
		$hover        = $params->get('fusion_gantt_chart_hover');
		$hover        = FStringHelper::safeColNameToArrayKey($hover);
		$milestone    = $params->get('fusion_gantt_chart_milestone');
		$milestone    = FStringHelper::safeColNameToArrayKey($milestone);
		$milestoneRaw = $milestone . '_raw';
		$connector    = $params->get('fusion_gantt_chart_connector');
		$connector    = FStringHelper::safeColNameToArrayKey($connector);
