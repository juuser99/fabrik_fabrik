...
namespace Fabrik\Plugin\FabrikVisualization\Chart\Model;
 ...
 * @since       4.0
 */
class ChartModel extends AbstractVisualizationModel
{
	/**
	...
	public function getChart()
	{
		...
		foreach ($tableDatas as $tableData)
		{
			...
			$safename   = FStringHelper::safeColName($column);
				...
				foreach ($alldata as $group)
				{
					foreach ($group as $row)
					{
						if (!array_key_exists($column, $row))
						{
							...
							if (!array_key_exists($column, $row))
							{
								Log::add($column . ': NOT FOUND - PLEASE CHECK IT IS PUBLISHED', Log::WARNING, 'jerror');
